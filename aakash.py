...
org_dict = {'Misc': {'total_contributions': 0, 'unique_contributors': 0, "id": [],
                     'date_range_parameter': f'{date_range_parameter}', 'Total Commit': 'total_commits'}}
n = 1
total_commits = 0
while True:
    ...
    # If organization is not users.noreply.github.com, make a new key as company name.
        ...
        if company in org_dict.keys():
            ...
            org_dict[company] = {'total_contributions': 1, 'unique_contributors': 0, 'id': [],
                                 'date_range_parameter': f'{date_range_parameter}'}
...
# s=0
# for i in org_dict:
#     conn = mysql.connector.connect(host='localhost', user='root', password='', database='APIDB')
#     cur = conn.cursor()
#     s=+1
#     cur.execute("INSERT INTO APIDB.mydb (UNIQUE_ID, REPO_NAME, DOMAIN_NAME, TOTAL_CONTRI, UNIQUE_CONTRI, COMMIT_DATE, DATE_OF_RUN) values(%d,%s,%s,%d,%s,%s,%s)",
#     (str(s),str(org_dict[i]['total_contributions']), str(org_dict[i]['unique_contributors']), str(i)))
#     conn.commit()
